...
IS
BEGIN
   RETURN('New^Partially Approved^Approved^Released^PartiallyTransfered^Transfered^Completed^');
...
BEGIN
   ...
   ELSIF (state_ = 'New') THEN
      IF (event_ = 'PartiallyApprove') THEN
...
IS
BEGIN
   db_events_ := 'Approve^Complete^PartialTransfer^PartiallyApprove^Release^Transfer^';
