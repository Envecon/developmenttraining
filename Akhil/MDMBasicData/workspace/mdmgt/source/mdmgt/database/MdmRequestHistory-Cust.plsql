...
BEGIN
   ...
   newrec_.history_status := Get_Status___(newrec_.template_id , newrec_.revision);
...
FUNCTION Get_Status___ (
      template_id_     mdm_request_history_tab.template_id%TYPE,
      revision_  mdm_request_history_tab.revision%TYPE )  RETURN VARCHAR2
IS 
   temp_status_ VARCHAR2(50);
...
BEGIN
   OPEN get_status;
   FETCH get_status INTO temp_status_;
   CLOSE get_status;   
   RETURN temp_status_;
END Get_Status___;
